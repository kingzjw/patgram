...
if(empty($_GET["c"])) {
	exit("Please provide a class code.");
}
...
$result = mysqli_query($conn, "SELECT classId, name, description
		FROM class
...
<div class="panel panel-default example-verbs">
	...
	<div class="panel-body">
		...
		{{#tablebody ev 6}}
			<span class="label {{#if hl}}label-primary{{else}}label-default{{/if}}">{{ev}}</span>
		{{/tablebody}}
		</table>
	</div>
</div>
...
{{#each this}}
<span class="label label-primary class-verb">{{this}}</span>
{{/each}}
</script>
...
<script>
	$(document).ready(function() {
		...
		var source = $("#keywords-template").html();
		var template = Handlebars.compile(source);
		var html = template(loadKeywordsFrom());
...
<body>
	...
	<section>
		<div class="container-fluid">
			<div>
				<header>
					<h2><i class="fa fa-university"></i> <?php echo $className; ?> <small><?php echo $description; ?></small></h2>
				<header>
				<h3 id="keywords"></h3>
			</div>
